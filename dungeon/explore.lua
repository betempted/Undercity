...
local explored_buffer = 3      -- Buffer around explored areas in meters
...
local function find_objective()
    ...
    for _, actor in ipairs(actors) do
        ...
        if actor_name:find("Switch") then
            return actor
        end
    end
    return nil
...
local path_recalculation_interval = 10.0 -- Recalculate path every 2 seconds
...
local function move_to_target()
    if target_position then
        ...
        if not current_path or #current_path == 0 or path_index > #current_path then
            ...
            Explore.set_custom_target(target_position)
        ...
        if calculate_distance(player_pos, target_position) < 4 then
...
local switch_positions_reached = {}

function Explore.run()
    local objective = find_objective()
    local player_pos = get_player_position()
    if objective then
        local objective_pos = objective:get_position()
        local switch_pos_key = objective_pos:x() .. "_" .. objective_pos:y() .. "_" .. objective_pos:z()
        if calculate_distance(player_pos, objective_pos) < 4 then
            if objective:is_interactable() then
                interact_object(objective)
                switch_positions_reached[switch_pos_key] = true
                console.print("Interacted with switch")
                return
            end
        else
            if not switch_positions_reached[switch_pos_key] then 
                Explore.set_custom_target(objective_pos)
                console.print("Setting custom target to switch")
            end
        end
    -- else
    --     local target_mob = target_selector.get_target_closer(player_pos, 40)
    --     if target_mob then
    --         if target_mob:is_elite() or target_mob:is_champion() or target_mob:is_boss() then
    --             Explore.set_custom_target(target_mob:get_position())
    --             console.print("Setting custom target to mob")
    --         end
    --     end
    end
    move_to_target()
